...
m <- lmer(nR1A ~ cValence + (1+cValence|Number) + (1+cValence|Participant), data = dcen)
summary(m)
```
...
```{r}
model_clmm <- clmm(R1A ~ cValence + (1+cValence|Number) + (1+cValence|Participant), data = dcen, link = "logit")
